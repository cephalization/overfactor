...
## 2026-08-19 — implemented — first-run desktop onboarding

The desktop app opens a durable three-step fullscreen onboarding flow until `config.json` records `onboarding.completed: true`: product/prerequisite overview, selectable Claude Code/Pi integration setup, and a live mock explaining repo/branch grouping plus curated review. Onboarding is intentionally evaluated **before daemon discovery**, so a first launch with no daemon still gets the product tour; finishing then reveals the existing daemon-offline state. Settings can clear the completion flag to replay the flow, and the sidebar's Plugins button reopens integration management without replaying the tour. The visual treatment is renderer-native (CSS fades/drifting ambient color, reduced-motion fallback) and stays within the app shell's existing token system.
...
- Pi reviews run the user's own `pi -p` with an explicit provider and model, no session, no tools, no context files/skills/templates, and no extensions. Disabling extensions is intentional isolation (and prevents the Overfactor Pi extension from reporting a phantom review session); provider/model entries from Pi's built-in catalog and `models.json` remain available, but providers registered only by another Pi extension are not available to this isolated runner.
- Pi model suggestions come from Pi 0.82.1's `ModelRuntime.getAvailable()` rather than parsing the human-oriented `--list-models` table. A real `openai-codex/gpt-5.6-sol` print-mode smoke test confirmed stdin produces clean final JSON with the isolation flags used by the engine.
- `~/.overfactor/config.json` now persists one explicit review policy: agent, provider, and model. Existing configs default to Claude Code + `sonnet`; Pi requires a non-empty provider and model. Automatic PR generation and Generate/Regenerate all read this policy at trigger time, so a settings save cannot race stale daemon memory. Reviews persist the effective provider alongside engine/model; existing database rows migrate with `provider = null`.
- The desktop Settings page is opened from the sidebar gear, selects Claude Code or Pi, suggests authenticated Pi providers/models while still accepting `models.json` entries manually, and is kept fresh through a `settings` WS invalidation. Per-review model entry was removed from the review pane so there is one predictable global default instead of two competing policies.
